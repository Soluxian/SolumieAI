#!/usr/bin/env python3
import os
import base64
import subprocess
import time
import json

SNAP = '/tmp/emu-snap.png'
JSON_TMP = '/tmp/vision-dexie.json'

SEL_GEOM = '420x281+30+62'

VALID_KEYS = set('WASDZXQERV')

def get_emu_window():
    try:
        wid = subprocess.check_output(['xdotool', 'search', '--name', 'mGBA']).decode().strip().split()[0]
        return wid
    except:
        return None

def snap_game():
    cmd = f"scrot {SEL_GEOM} {SNAP}"
    print(f'Exec: {cmd}')
    os.system(cmd)
    if os.path.exists(SNAP):
        size = os.path.getsize(SNAP)
        print(f'Snap OK {size}b')
    else:
        print('Snap FAIL—manual scrot {SEL_GEOM} {SNAP}')
        os.system(f"scrot -s {SNAP}")

def vision_move():
    wid = get_emu_window()
    if not wid:
        print('No emu.')
        return
    os.system(f'xdotool windowactivate --sync {wid}; sleep 1')
    snap_game()
    if not os.path.exists(SNAP):
        return

    with open(SNAP, 'rb') as f:
        img_b64 = base64.b64encode(f.read()).decode()[:800000]

    data = {{
        'model': 'moondream',
        'prompt': 'GBA Fire Emblem. BEST keys WASD Z X Q E V R ONLY e.g. DZ.',
        'images': [img_b64],
        'stream': False
    }}

    with open(JSON_TMP, 'w') as f:
        json.dump(data, f)

    print('Curl ollama...')
    res = subprocess.run(['curl', '-s', '-X', 'POST', 'http://localhost:11434/api/generate', '-d', f'@{JSON_TMP}', '--max-time', '25'], capture_output=True, text=True, timeout=30)
    os.unlink(JSON_TMP)

    print('Curl RC:', res.returncode)
    if res.returncode != 0:
        print('STDERR:', res.stderr)
        return

    try:
        response = json.loads(res.stdout)['response'].upper()
    except:
        response = res.stdout.upper()[:50]

    print('Vision:', repr(response))
    clean_keys = ''.join(c for c in response if c in VALID_KEYS) or 'Z'
    print(f'Keys: {clean_keys}')

    for k in clean_keys:
        os.system(f'xdotool key {k}; sleep 0.25')
    print('Cycle complete.')

print('v16 | SCROT FIX + VERBOSE + TIMEOUT')
print('Emu ready. ENTER.')
input()

while True:
    print('\n--- CYCLE ---')
    vision_move()
    time.sleep(2)